...
set finalAltitude to 47500.		// Altitude in meters at the end of the gravity turn
set finalPitch to 90.			// Absolute pitch angle in degrees at the end of the gravity turn
...
set beta3FuelStage1 to   0.0.
set beta3FuelStage2 to   0.0.
set beta3FuelStage3 to   0.0.

// Standard local variables
set fuelStage1 to 0.0.
set fuelStage2 to beta3FuelStage2.
set fuelStage3 to beta3FuelStage3.

list engines in englist.
for eng in englist {
	print "An engine exists with thrust = " + eng:thrust.
}

set twrStage3 to 0.47.					// see if these twr variables can be pulled from API
set twrStage2 to 1.88.
set twrStage1 to 1.82.
set twrAtmo to 1.4.
set twrUpperAtmo to 1.5.
...
until ship:altitude > finalAltitude {
	...
	if stage:liquidfuel < (fuelStage1 + 0.1) and currentStage = 1 {
		print "Stage 1 fuel         : " + stage:liquidfuel at (0,17).	// Added for apparent kOS bug
	...
	if stage:liquidfuel < (fuelStage2 + 0.1) and currentStage = 2 {
		print "Stage 2 fuel         : " + stage:liquidfuel at (0,18).	// Added for apparent kOS bug
		...
		stage.
		wait 0.2.
		set throttleLimit to max (1.0 , (1.0 / twrStage3)).
...
print "Ignition at: " + time:clock + " UT".
lock throttle to 1.0.
if stage:liquidfuel < (fuelStage3 + 0.1) and currentStage = 2 {
	print "Stage 2 fuel         : " + stage:liquidfuel at (0,20).	// Added for apparent kOS bug
...
print "Orbit!".
lock throttle to 0.
print "Cutoff at " + time:clock + " UT".

if stage:liquidfuel < (fuelStage3 + 0.1) and currentStage = 2 {
// if currentStage = 2 {
	// print "Stage 2 seperation at  : " + ship:altitude +"m" at (0,22).
	// print "Stage 2            at  : " + time:clock + " UT" at (0,23).
	print "Stage 2 fuel remaining : " + stage:liquidfuel at (0,24).	// Added for apparent kOS bug
	stage.
	set currentStage to 3.
}
