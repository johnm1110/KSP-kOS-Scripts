...
set booster to "Atlas2".
run launcher.

// load payload
set payload to "CommSat".
run payloads.

set SHIP:SHIPNAME to "CommSat Kerbin G1".
...
until program = 0 {
	...
	if program = 12 { // Trans Munar Injection (TMI) burn
		...
		until timeUntilBurn < 0 {
			...
			print "Total delta V required (m/s)     : " + round(dVHohmann,2) + "          " at (0,5).
			print "Total time of burn (s)           : " + round(burnTime,2) + "          " at (0,6).
			print "Apoapsis of transfer ellipse (m) : " + rAp at (0,7).
			wait 0.001.
		}
		set t0 to TIME:SECONDS.
		until (TIME:SECONDS - t0) > burnTime {
